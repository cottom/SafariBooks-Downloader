...
program
  ...
  .option('-k, --kfloder <kfloder>','folder')
  .parse(process.argv);

const baseDir = program.kfloder ? program.kfloder : path.resolve((process.env.HOME || process.env.HOMEPATH || process.env.USERPROFILE) + '/.safariDownloader');
console.log(baseDir);
const browserDataDir = baseDir + '/.browser-data'
const cookiesFile = baseDir + '/.cookies-file'
const credentialsFile = baseDir + '/.credentials'
...
// });
  ...
  safariClient.fetchBookById(program.bookid, username, password, cookies, program.weblogin).then( (bookJSON) => {
    ...
    var ebook = new EbookWriter(bookJSON);
    return ebook.save(path.resolve(program.output, `${bookJSON.title}.epub`));
  }).then( () => {
    // ## finished saving
    debug("the epub was successfully saved");
    logger.log("epub successfully saved. exiting...");
