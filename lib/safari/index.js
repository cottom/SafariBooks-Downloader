...
// # import dependencies
const queuely = require('../util/queuefly');
const debug = require('debug')('safari'),
      logger = require('../logger');
const request = queuely(require('request-promise'));
...
*/
Safari.prototype.fetchResource = function fetchResource(url, options, retry = 3) {
  ...
  return request(settings).then( (body) => {
    ...
    // ### an error occurred
    if (retry) {
      debug(`[error][retry ${retry}] there was an unexpected error fetching the resource (err: ${err})`)
      return this.fetchResource(url, options, retry--);
    } else {
      debug(`[error][fail] there was an unexpected error fetching the resource (err: ${err})  (context: ${uri})`)
      return Promise.reject(err);
    }
  });
}
